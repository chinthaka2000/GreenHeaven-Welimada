...
export function Accommodations() {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      const { data } = await supabase.from('site_settings').select('accommodations_opening_soon').single();
      if (data && data.accommodations_opening_soon) {
        setOpeningSoon(true);
      }
    };
    fetchSettings();
  ...
  return (
    <Section id="accommodations">
      ...
      <div className="relative">
        ...
        <div className={`space-y-16 md:space-y-32 transition-all duration-1000 ${openingSoon ? 'blur-md opacity-60 pointer-events-none select-none' : ''}`}>
          {rooms.map((room, idx) => (
            <motion.div
              key={room.name}
